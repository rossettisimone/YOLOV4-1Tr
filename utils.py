...
def show_infer(images, prediction, class_dict):
...
def unmold_mask_proposal(mask_bbox):
    mask, bbox = mask_bbox
    bbox = tf.cast(bbox, tf.int32)
    mask = mask[...,None]
    size = bbox[2:4]-bbox[0:2]
    size = size[::-1]
    target_height, target_width = cfg.TRAIN_SIZE, cfg.TRAIN_SIZE
    if tf.reduce_all(size>0):
        offset_width, offset_height = bbox[0], bbox[1]
        mask = tf.image.resize(mask, size, method=tf.image.ResizeMethod.BILINEAR)
        mask = tf.image.pad_to_bounding_box(mask, offset_height, offset_width, target_height, target_width)[...,0]
        mask = tf.where(mask>=0.5,1.0,0.0)
    else:
        mask = tf.zeros((target_height, target_width))
    return mask

    
def unmold_mask_proposals(masks_bboxes):
    masks = tf.map_fn(unmold_mask_proposal, masks_bboxes, fn_output_signature = tf.float32)
    return masks
    
def unmold_mask_batch(masks, bboxes):
    masks = tf.map_fn(unmold_mask_proposals, (masks, bboxes), fn_output_signature = tf.float32)
    return masks
...
#    ids = tf.math.subtract(target[:,4],1.0)
    ...
    bg_index = tf.logical_and(iou_map < cfg.BG_THRESH * tol , tf.logical_not(point_mask))
    ...
    tid = tf.where(bg_index[...,None], 0.0, tid)
...
#    pconf = tf.nn.softmax(pconf, axis=-1)[...,1:] # class
    pclass = prediction[..., 4:]  # class
    pclass = tf.nn.softmax(pclass, axis=-1)[...,1:] # class
    pbox = prediction[..., :4]
    pbox = decode_delta_map(pbox, tf.divide(anchors,stride))
    pbox = tf.multiply(pbox,stride) # now in range [0, .... cfg.TRAIN_SIZE]
    pbox = tf.divide(pbox,cfg.TRAIN_SIZE) #now normalized in [0...1]
    pbox = xywh2xyxy(pbox) # to bbox
    pbox = tf.clip_by_value(pbox,0.0,1.0) # clip to avoid nan
    proposal = tf.concat([pbox, pclass], axis=-1) #pconf
    nB = tf.shape(proposal)[0]
    _, nA, nGh, nGw, nC = proposal.shape       
...
def check_proposals_tensor(proposal):
    ...
    pconf = tf.reduce_max(proposal[...,4:], axis=-1)#proposal[..., 4]
  ...
  """
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for data in self.val_ds.take(self.iters):
            ...
            mask = mask  * tf.tile(gate[...,None,None],(1,1,28,28))
