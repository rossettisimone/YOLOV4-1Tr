...
def  gen_hist(bbs, image, frame_hist, edges, len_edges, k): # 
    ...
    rem_imm = np.zeros((len(bbs),2*cfg.LENH +cfg.FF +  4) )
    ...
    for tt, bb in enumerate(bbs):
        ...
        if  not any(np.array(bb)>0):
           hhX =np.random.rand(1, 2*cfg.LENH +cfg.FF +  4)
           # print('faked')
           ...
           # bbx = bb_int.copy()
            ...
            labFeat = rgb2lab(features)
            labFeat = (labFeat + [0, 128, 128]) / [100, 255, 255]
            labFeat = (labFeat + abs(np.min(labFeat)))/np.max(labFeat)
        ##
            hh = np.zeros((len_edges-3, 3))
            ...
            hhL, hh = color_histogram(hh, labFeat[:,:,0], labFeat[:,:,1], labFeat[:,:,2], edges, len_edges)
            hhf, hh = color_histogram(hh, features[:,:,0],features[:,:,1],features[:,:,2], edges, len_edges)
        
            if type(bb) == list:
               np.array(bb)
            hhX = np.concatenate([hhf, hhL, F1, 2*(bb/(np.sum(bb)))])
        rem_imm[tt,:] = hhX # 
...
def compare_frames(frame1, frame2,p):
    ...
    #m = frame2.shape[0]
    ## Here we assume frame1 shape[0]=1
    ...
    list_corr = np.zeros((m,d))
    for u, ff in enumerate(frame1):
        ff_frame = matlib.repmat(ff,d,1)
        corri =  np.array(tfp.stats.correlation(ff_frame,frame2, sample_axis=1, event_axis = None))
        if any(np.isnan(corri)):
              corri[np.isnan(corri)]  = 0
        list_corr[u,:]= corri
...
# def resize_eq(sh0,sh1):
#      x = np.max(sh0[0], sh0[1])
#      y = np.max(sh0[0], sh0[1])
