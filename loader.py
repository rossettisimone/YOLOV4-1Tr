...
class Generator(object):
    ...
    def _single_input_generator_train(self, index):
        ...
        image, masks, mini_masks, bboxes, good_sample = self.data_generator(video, frame_id)
        if good_sample:
            if self.augment:
                image, masks, bboxes = self.data_augment(image, masks, bboxes)
            image, masks, bboxes = self.data_preprocess(image, bboxes, masks)
            mini_masks = self.mini_masks_generator(masks, bboxes)
            masks, mini_masks, bboxes = self.data_check(masks, mini_masks, bboxes)
    ...
    def data_check(self, masks, mini_masks, bboxes):#check consistency of bbox after data augmentation: dimension and ratio
    ...
    def mini_masks_generator(self, masks, bboxes):
        masks_resized = []
