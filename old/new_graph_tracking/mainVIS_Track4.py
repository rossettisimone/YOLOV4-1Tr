...
"""
#import sys 
#sys.path.append('/home/fiorapirri/tracker/new_graph_tracking')
from new_graph_tracking.util import file_reader, rle_decoding, draw_bbox,  get_whole_single_video
...
from new_graph_tracking.subcompare import subcompare_frames, search_X_extras, main_search 
import config
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%% LOAD %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for FROM_idx in ids:
    ...
    print('starting video', video_X[0]['annotation_id'],' ->', FROM_idx+1,'/',len(idx))
...
#    non =   [i for i,X in enumerate(video_X) if len(X.get('pred_bboxes' ))>0]
    ...
    idx_objs = main_search(bboxes, imms, idx_objs, max_preds, edges, len_edges, video_X)
...
with open('results_test_new_graph.json', 'w') as f:
